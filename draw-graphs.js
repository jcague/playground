...
function addGraphCanvas(container, id, title) {
...
function show(stream, isRemote) {
  const id = isRemote ? stream.getVideoTracks()[0].id : "local";
  ...
  addGraphCanvas(container, id + "BitrateCanvas", "Bitrate");
  ...
  addGraphCanvas(container, id + "FramerateCanvas", "Framerate");
  ...
  addGraphCanvas(container, id + "qpCanvas", "qp");
  ...
  addGraphCanvas(
    container,
    id + "BandwidthCanvas",
    "qualityLimitationDurations(BW)"
  );

  const bandwidthGraph = new TimelineGraphView(
    id + "Container",
    id + "BandwidthCanvas"
  );
  bandwidthGraph.updateEndDate();

  bandwidthSeries[id] = id === "local" ? new Map() : new TimelineDataSeries();
  bandwidthGraphs[id] = bandwidthGraph;
...
async function draw(pc1, pc2) {
  ...
  res1.forEach((report) => {
    ...
    const bwLimit = report.qualityLimitationDurations?.bandwidth || 0;
    ...
    if (lastSendResult && lastSendResult.get(report.id)) {
      ...
      if (!bandwidthSeries[graphName].has(ssrc)) {
        const series = new TimelineDataSeries();
        bandwidthSeries[graphName].set(ssrc, series);
        if (bandwidthSeries[graphName].size === 2) {
          series.setColor("green");
        } else if (bandwidthSeries[graphName].size === 3) {
          series.setColor("blue");
        }
      }
      bandwidthSeries[graphName].get(ssrc).addPoint(now, bwLimit);

      // calculate qp-per-frame
      if (report.qpSum) {
        const qp =
          (report.qpSum - lastSendResult.get(report.id).qpSum) /
  ...
  if (document.getElementById("local_currentData")) {
    document.getElementById("local_currentData").innerText =
      "Stream: local\n" +
      Object.keys(sentKeyframes)
        .sort()
        .map((rid) => {
          return (
            "rid=" +
  ...
  bitrateGraphs["local"].setDataSeries(
    Array.from(bitrateSeries["local"].values())
  );
  bitrateGraphs["local"].updateEndDate();

  framerateGraphs["local"].setDataSeries(
    Array.from(framerateSeries["local"].values())
  );
  framerateGraphs["local"].updateEndDate();

  bandwidthGraphs["local"].setDataSeries(
    Array.from(bandwidthSeries["local"].values())
  );
  bandwidthGraphs["local"].updateEndDate();
  ...
  res2.forEach((report) => {
    ...
    if (lastRecvResult && lastRecvResult.get(report.id)) {
      ...
      if (adapter.browserDetails.browser === "firefox") {
        ...
        graphName =
          report.trackIdentifier || res2.get(report.trackId).trackIdentifier;
      }
      ...
      if (report.qpSum) {
        const qp =
          (report.qpSum - lastRecvResult.get(report.id).qpSum) /
      ...
      bandwidthSeries[graphName].addPoint(now, bwLimit);
      bandwidthGraphs[graphName].setDataSeries([bandwidthSeries[graphName]]);
      bandwidthGraphs[graphName].updateEndDate();
